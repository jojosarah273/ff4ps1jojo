...
def parse_load_or_store(rest: str):
    if match := re.match(
        r"(\$[a-z0-9]+),\s*%(lo|gp_rel|got)\(([^(]+)\)\(([^(]+)\)", rest
    ):
        # %lo/%gp_rel/%got(x)(base): reloc forms only the assembler can resolve.
        # Leave these untouched for GNU as (can't %hi()/%lo() an operator).
        r_dest, operand, r_source = match.group(1, 3, 4)
    ...
    def preprocess_lines(self) -> None:
        ...
        for line in self.lines:
            ...
            if line.startswith(".extern"):
                # e.g. .extern D_8019ED40, 4  (CC1PSX output)
                # ASPSX treats small externs as gp-relative sbss members:
                _, symbol, size_str = line.split()
